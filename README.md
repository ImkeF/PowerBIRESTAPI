...
5. Select all the files inside the unzipped folder and zip it again, but now change the extension from *.zip* to *.mez*. (Make sure to select all files and not the folder to zip back. Otherwise a error message like "We couldn't find the AppId.txt" could occur.)
